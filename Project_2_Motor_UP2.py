...
df = df[['subject#', 'age', 'sex', 'test_time','jitter(%)','jitter(abs)',	
        ...
        'shimmer(dda)', 'nhr', 'hnr', 'rpde', 'dfa', 'ppe', 'total_updrs', 'motor_updrs']]
...
X_motor = df.iloc[:, 1:-1].values
y_motor = df.iloc[:,-1].values

X_motor_train, X_motor_test, y_motor_train, y_motor_test = train_test_split(X_motor, y_motor, train_size = 0.8, random_state = 0)
...
axs_2[2,1].scatter(x = df['total_updrs'], y = df['motor_updrs'])
axs_2[2,1].set_xlabel('Total UPDRS')
axs_2[2,1].set_ylabel('motor_UPDRS')

df_motor = df.drop(['subject#'], axis=1)
corr = df_motor.corr()
...
#Basic Model for motor UPDRS Prediction without motor UPDRS

print('\n----------Basic Model for motor UPDRS Prediction with total UPDRS----------\n')
...
print('\n-----------Log Transformation for motor UPDRS Prediction with total UPDRS----------\n')
...
df_motor_LT = df_motor_LT[['LAge', 'sex', 'test_time','LJitter(%)','LAbs Jitter',	
        ...
        'LShimmer DDA', 'LNHR', 'LHNR', 'rpde', 'dfa', 'ppe', 'total_updrs', 'motor_updrs']]
...
print('\n-----------Colinearity Analysis for motor UPDRS Prediction with total UPDRS----------\n')

df_motor_CL = df_motor.copy()

df_motor_CL = df_motor_CL.drop(['jitter(rap)', 'jitter(ppq5)', 'jitter(ddp)', 
...
print('\n-----------Z-Standardization for motor UPDRS Prediction with total UPDRS----------\n')
...
print('\n-----------Gaussian Transformation for motor UPDRS Prediction with total UPDRS----------\n')
