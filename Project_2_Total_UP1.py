...
X_total = df.iloc[:, 1:-2].values
y_total = df.iloc[:, -2:-1].values
...
df_total = df.drop(['motor_updrs', 'subject#'], axis=1)
corr = df_total.corr()
...
print('\n----------Basic Model for total UPDRS Prediction without Motor UPDRS----------\n')
...
print('\n-----------Log Transformation for total UPDRS Prediction without Motor UPDRS----------\n')
...
df_total_LT[['LAge', 'LJitter(%)', 'LAbs Jitter', 'LJitter RAP', 'LJitter PPQ5', 'LJitter DDP',
             ...
             'LShimmer DDA', 'LNHR']] = df_total_LT[columns_to_log].apply(lambda x: np.log(x))
...
df_total_LT = df_total_LT[['LAge', 'sex', 'test_time','LJitter(%)','LAbs Jitter',	
        ...
        'LShimmer DDA', 'LNHR', 'hnr', 'rpde', 'dfa', 'ppe', 'total_updrs']]
...
print('\n-----------Colinearity Analysis for total UPDRS Prediction without Motor UPDRS----------\n')

df_total_CL = df_total.copy()

df_total_CL = df_total_CL.drop(['jitter(%)', 'jitter(rap)', 'jitter(ppq5)', 'jitter(ddp)', 
                    'shimmer(abs)', 'shimmer(apq3)', 'shimmer(apq5)', 'shimmer(dda)'],
                    axis = 1)
...
print('\n-----------Z-Standardization for total UPDRS Prediction without Motor UPDRS----------\n')
...
print('\n-----------Gaussian Transformation for total UPDRS Prediction without Motor UPDRS----------\n')
